...
export default async function PrivacyRoute() {
  const [site, location, page] = await Promise.all([
    getSiteData(),
    getLocationData(),
    getLegalPage("privacy-policy").catch(() => undefined),
  ]);
  ...
  return (
    <main className="min-h-screen bg-[#FEF2F2] text-[#721011]">
      ...
      <article className="mx-auto max-w-4xl px-6 pb-20 pt-32 sm:pt-40">
        <h1 className="font-heading text-5xl font-bold">{page?.title || "Privacy Policy"}</h1>
        <div className="mt-8 space-y-5 text-lg leading-8 text-[#721011]/75">
          {(paragraphs.length ? paragraphs : ["This page is editable in Payload CMS. Add your Australian privacy policy content in the Legal Pages collection."]).map((paragraph) => (
